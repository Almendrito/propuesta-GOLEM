...
def analyze_mirnov_discharge_headless(shot, t_window_spec=(4e-3, 16e-3), t_window_psd=(8e-3, 13e-3),
                                      t_window_coh=(10e-3, 12.5e-3), f_band_mhd=(20e3, 40e3),
                                      coils_active=None, spec_vmin=0.2, spec_vmax=1.0, coh_threshold=0.7):
    ...
    X_proc_spec = {c: preprocess_signal(U_corr_full[c], fs_global) for c in valid_coils}
    ...
    if others:
        ...
        for j, c2_name in enumerate(others):
            ...
            is_valid = np.mean(Cxy_masked) > coh_threshold if len(Cxy_masked) > 0 else False
            if is_valid:
                valid_pts = Cxy_masked > coh_threshold
                if not np.any(valid_pts): valid_pts = np.ones(len(Cxy_masked), dtype=bool)
    ...
    if len(phase_data_calc) >= 3:
        ...
        x_arr, y_arr = np.array(x_fit), np.array(y_fit)
        m_candidates = np.arange(-6, 7)
        best_m = 0
        min_error = float('inf')
        
        for m_test in m_candidates:
            err = np.sum(np.abs(np.exp(1j * y_arr) - np.exp(1j * m_test * x_arr))**2)
            if err < min_error:
                min_error, best_m = err, m_test
        ...
        rmse_pi = np.sqrt(np.mean((y_unwrapped - (m_fine * x_arr))**2)) / np.pi
        ...
        for i, name in enumerate(names_found):
             ax3.text(x_arr[i]/np.pi, y_unwrapped[i]/np.pi + 0.1, name, ha='center', fontsize=9, color='blue')
...
class MirnovAppToplevel:
    def __init__(self, root, shot_number):
        self.root = root
        self.root.title(f"Golem Mirnov Analysis Tool - Shot {shot_number}")
        ...
        self.shot_var = tk.IntVar(value=shot_number)
        ...
        self.canvas_spec = None; self.canvas_coh = None; self.canvas_mode = None
        self.toolbar_spec = None; self.toolbar_coh = None; self.toolbar_mode = None
    ...
    def _create_controls(self, parent):
        row = 0
        ttk.Label(parent, text=f"Shot Number: {self.shot_var.get()}", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w", pady=5); row+=1
        ttk.Separator(parent, orient=tk.HORIZONTAL).grid(row=row, column=0, sticky="ew", pady=10); row+=1

        ttk.Label(parent, text="Spectrogram Window (ms):", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w"); row+=1
        f_spec = ttk.Frame(parent); f_spec.grid(row=row, column=0, sticky="ew")
        ...
        ttk.Entry(f_spec, textvariable=self.t_spec_end, width=8).pack(side=tk.LEFT, padx=2); row+=1

        ttk.Label(parent, text="Spec Color Range (vmin/vmax):", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w", pady=5); row+=1
        f_clim = ttk.Frame(parent); f_clim.grid(row=row, column=0, sticky="ew")
        ttk.Entry(f_clim, textvariable=self.spec_vmin, width=8).pack(side=tk.LEFT, padx=2)
        ttk.Label(f_clim, text="to").pack(side=tk.LEFT)
        ttk.Entry(f_clim, textvariable=self.spec_vmax, width=8).pack(side=tk.LEFT, padx=2); row+=1

        ttk.Label(parent, text="PSD Window (ms):", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w", pady=5); row+=1
        f_psd = ttk.Frame(parent); f_psd.grid(row=row, column=0, sticky="ew")
        ttk.Entry(f_psd, textvariable=self.t_psd_start, width=8).pack(side=tk.LEFT, padx=2)
        ttk.Label(f_psd, text="to").pack(side=tk.LEFT)
        ttk.Entry(f_psd, textvariable=self.t_psd_end, width=8).pack(side=tk.LEFT, padx=2); row+=1

        ttk.Label(parent, text="Coherence Window (ms):", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w", pady=5); row+=1
        f_coh = ttk.Frame(parent); f_coh.grid(row=row, column=0, sticky="ew")
        ttk.Entry(f_coh, textvariable=self.t_coh_start, width=8).pack(side=tk.LEFT, padx=2)
        ttk.Label(f_coh, text="to").pack(side=tk.LEFT)
        ttk.Entry(f_coh, textvariable=self.t_coh_end, width=8).pack(side=tk.LEFT, padx=2); row+=1

        ttk.Label(parent, text="MHD Freq Band (kHz):", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w", pady=5); row+=1
        f_band = ttk.Frame(parent); f_band.grid(row=row, column=0, sticky="ew")
        ttk.Entry(f_band, textvariable=self.f_mhd_low, width=8).pack(side=tk.LEFT, padx=2)
        ttk.Label(f_band, text="to").pack(side=tk.LEFT)
        ttk.Entry(f_band, textvariable=self.f_mhd_high, width=8).pack(side=tk.LEFT, padx=2); row+=1

        ttk.Label(parent, text="Coh Threshold (>):", font=('Arial', 10, 'bold')).grid(row=row, column=0, sticky="w", pady=5); row+=1
    ...
    def on_run(self):
        ...
        try:
            ...
            return
        threading.Thread(target=self.run_logic, args=(params,)).start()
    ...
    def update_gui(self, results):
        ...
        def embed_figure(fig, parent, canvas_attr, toolbar_attr):
            ...
            if old_toolbar: old_toolbar.destroy()
